...
(function () {
  ...
  function indexContent() {
    ...
    state.allItems = (state.content.items || []).slice().sort(function (a, b) {
  ...
  function rebuildPlaylist(keepIndex) {
    ...
    else pl = state.allItems.filter(function (it) { return it.section === v.section; }).sort(byDisplayOrder);
  ...
  function goToItem(id) {
    ...
    state.playlist = state.allItems.filter(function (x) { return x.section === it.section; }).sort(byDisplayOrder);
    state.index = 0;
